...
class WanAnimate:
    ...
    def __init__(
        ...
        Args:
            ...
            quantized_dit_dir (`str`, *optional*, defaults to None):
                Path to pre-quantized NF4 DiT model. If set, loads quantized
                model instead of full-precision from checkpoint_dir.
        """
        ...
        self.quantized_dit_dir = quantized_dit_dir
        ...
        # Store paths only — models are loaded one at a time in generate()
        self._t5_args = dict(
            ...
            shard_fn=partial(shard_model, device_id=device_id) if t5_fsdp else None,
        )
        self._clip_args = dict(
            dtype=torch.float16,
            device=self.device,
            checkpoint_path=os.path.join(checkpoint_dir, config.clip_checkpoint),
            tokenizer_path=os.path.join(checkpoint_dir, config.clip_tokenizer),
        )
        self._vae_pth = os.path.join(checkpoint_dir, config.vae_checkpoint)
        ...
        logging.info(f"WanAnimate initialized (lazy loading). checkpoint_dir={checkpoint_dir}")
    ...
    def generate(
        ...
        # ── T5 encode (load → encode → delete) ──
        logging.info("Loading T5...")
        text_encoder = T5EncoderModel(**self._t5_args)
        if not self.t5_cpu:
            text_encoder.model.to(self.device)
            context = text_encoder([input_prompt], self.device)
            context_null = text_encoder([n_prompt], self.device)
        else:
            context = text_encoder([input_prompt], torch.device('cpu'))
        ...
        while True:
            ...
            for key, value in batch.items():
                if isinstance(value, torch.Tensor):
                    batch[key] = value.to(device=self.device, dtype=self.param_dtype)
            ...
            with (
                torch.autocast(device_type=str(self.device), dtype=self.param_dtype, enabled=True),
                torch.no_grad()
            ):
                ...
                # ── VAE encode phase (load → encode → delete) ──
                logging.info("Loading VAE for encode...")
                vae = Wan2_1_VAE(vae_pth=self._vae_pth, device=self.device)

                pose_latents_no_ref = vae.encode(conditioning_pixel_values.to(self.param_dtype))
                ...
                ref_latents = vae.encode(ref_pixel_values.to(self.param_dtype))
                ref_latents = torch.stack(ref_latents)

                mask_ref = self.get_i2v_mask(1, lat_h, lat_w, 1, device=self.device)
                y_ref = torch.concat([mask_ref, ref_latents[0]]).to(dtype=self.param_dtype, device=self.device)
                ...
                # ── DiT denoise phase (load → denoise → delete) ──
                logging.info("Loading DiT...")
                if self.quantized_dit_dir is not None:
                    noise_model = WanAnimateModel.from_pretrained(
                        self.quantized_dit_dir,
                        device_map="auto",
                ...
                noise_model = self._configure_model(
                    model=noise_model,
                    use_sp=self.use_sp,
                    dit_fsdp=self.dit_fsdp,
                    shard_fn=partial(shard_model, device_id=self.device.index or 0),
                    convert_model_dtype=False,
                    use_lora=self.use_relighting_lora,
                ...
                for i, t in enumerate(tqdm(timesteps)):
                    ...
                    noise_pred_cond = TensorList(
                         noise_model(TensorList(latent_model_input), t=timestep, **arg_c)
                    )
                ...
                logging.info("Loading VAE for decode...")
                vae = Wan2_1_VAE(vae_pth=self._vae_pth, device=self.device)
